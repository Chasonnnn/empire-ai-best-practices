...
| Job pends for hours/days, then fails in **seconds** at its own preflight (manifest/schema mismatch, missing input) | a staged artifact predates a code or schema upgrade — validation only ran at job start, so the whole queue wait was spent discovering it | regenerate the staged artifact; then prevent the class: run the job's CPU-safe preflight checks at submit time on the login node via the same code path (an `--io-only` flag), and whenever a validator/schema changes, regenerate already-staged artifacts in the same change. A still-PENDING job's inputs can be fixed in place — they're read at job start, so queue position survives |
